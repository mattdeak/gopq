...
func nackImpl(db *sql.DB, tableName string, id int64, opts AckOpts, deadLetterQueue Enqueuer) error {
	...
	err = tx.QueryRow(fmt.Sprintf("SELECT retry_count, ack_deadline FROM %s WHERE id = ?", tableName), id).Scan(&retryCount, &currentAckDeadline)
	if err != nil {
	...
	if retryCount >= opts.MaxRetries && opts.MaxRetries != InfiniteRetries {
		if deadLetterQueue != nil {
			var item []byte
			err = tx.QueryRow(fmt.Sprintf("SELECT item FROM %s WHERE id = ?", tableName), id).Scan(&item)
			if err != nil {
